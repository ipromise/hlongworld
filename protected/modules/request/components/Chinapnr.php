...
class Chinapnr{
	...
	const CMDID_QUERY_CARD_INFO= "QueryCardInfo"; //银行卡查询接口,后台数据流方式
        const CMDID_FUND_REGISTER= "FundRegister"; //银行卡查询接口,后台数据流方式
...
HTML;
	 ...
	 * @return 无返回，使用autoRedirect方式重定向用户浏览器页面
	 */
	public function fundRegister($merCustId,$usrCustId,$pageType='',$retUrl='',$bgRetUrl,$merPriv = '')
	{
		$checkValue= $this->sign(md5($this::VERSION_10.$this::CMDID_FUND_REGISTER.$merCustId.$usrCustId.$pageType.$retUrl.$bgRetUrl.$merPriv));
                $reqData=array(
				"Version"	=>	$this::VERSION_10,
				"CmdId"		=>	$this::CMDID_FUND_REGISTER,
				"MerCustId"	=>	$merCustId,
                                "UsrCustId"	=>	$usrCustId,
				"PageType"	=>	$pageType,
				"RetUrl"	=>	$retUrl,
